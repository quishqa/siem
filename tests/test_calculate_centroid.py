...
from siem.point import create_gpd_from, create_emiss_point, calculate_centroid
...
def test_calculate_centroid() -> None:
    ...
    assert isinstance(emiss_centroid, pd.DataFrame)
    assert "ID" not in emiss_centroid.columns
    assert "geometry" not in emiss_centroid.columns
    assert "centro" not in emiss_centroid.columns
