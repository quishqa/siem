...
scen        = 'test'                  # Scenario of emission factors and fleet
                                      # scenC represent the year 2018
...
# Read Data ===================================================================
proj_path   = '../tests/test_data/'
geopath     = proj_path + 'geo_em.'+dom+'.nc'
geo         = xr.open_dataset(geopath)
west_east   = len(geo.west_east)
south_north = len(geo.south_north)
proxy_ldv   = read_spatial_proxy(proj_path + "/highways_ldv_"+dom+".csv",(west_east, south_north))
proxy_hdv   = read_spatial_proxy(proj_path + "highways_hdv_"+dom+".csv",(west_east, south_north))

wrfinput      = xr.open_dataset(proj_path + "wrfinput_"+dom)
...
fleet        = 7_284_990  # Fleet circulation with survival curve
gasohol_fv   = 0.205400   # FRACAO VEICULOS MOVIDOS A GASOLINA (VEIC 1)          
ethanol_fv   = 0.014506   # FRACAO VEICULOS MOVIDOS A ETANOL (VEIC 2)           
flex_fv      = 0.561566   # FRACAO VEICULOS MOVIDOS A FLEX (VEIC 3)             
trucks_fv    = 0.060919   # FRACAO CAMINHOES (DIESEL - VEIC 4A)                 
urban_bus_fv = 0.005305   # FRACAO ONIBUS URBANO (DIESEL - VEIC 4B)                                    
intercity_fv = 0.001932   # FRACAO ONIBUS RODOVIARIO (DIESEL - VEIC 4C)         
gasobike_fv  = 0.107455   # FRACAO MOTOS MOVIDOS A GASOLINA (VEIC 6A)           
ethabike_fv  = 0.042916   # FRACAO MOTOS FLEX (VEIC 6B)  
...
road_transport = GroupSources(sources_list = [gasohol_ldv, ethanol_ldv, flex_ldv,
                                              res_ldv, trucks, urban_bus,
                                              intercity, res_hdv,
                                              gasohol_bike, ethanol_bike])


veh_emiss      = road_transport.to_cmaq(wrfinput = wrfinput,
...
# road_transport.report_emissions()
# road_transport.report_emissions().xs('PM', level = 'pol').sort_values('total_emiss')
